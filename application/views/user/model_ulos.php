...
<head>
    ...
    <link href="<?= base_url('assets/user/css/style6.css'); ?>" rel="stylesheet">
    <link href="<?= base_url('assets/user/css/tambahan1.css'); ?>" rel="stylesheet">
</head>
                ...
                <div class="col-lg-6">
                    <div class="owl-carousel owl-theme s_Product_carousel">
                        <div class="single-prd-item">
                            <img class="img-fluid" style="max-width: 100%;" src="<?php echo base_url('assets/upload/image/' . $get_detail_ulos->produk_gambar) ?>" alt="">
                        </div>
                    </div>
                </div>
                <div class="col-lg-5 offset-lg-1">
                    <div class="s_product_text">
                        <input type="hidden" id="nama_produk" name="nama_produk" value="<?php echo $get_detail_ulos->produk_nama ?>">

                        <h3><?php echo $get_detail_ulos->produk_nama ?></h3>
                        <input type="hidden" id="harga_per_produk" name="harga_per_produk" value="<?php echo $get_detail_ulos->produk_harga; ?>">

                        <h2 id="harga_produk" name="harga_produk" value="">Rp.<?php echo $get_detail_ulos->produk_harga; ?></h2>
                        <br>
                        <ul class=" list">
                            ...
                            <div class="controls">
                                <select id="model" name="model" class="form-control" required>
                                    ...
                                    <?php foreach ($model as $model) : ?>
                                        <option value="<?php echo $model->id_model; ?>"><?php echo $model->nama_model; ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            </br>
                            <div class="controls">
                                <input type="hidden" id="total_ulos" name="total_ulos" value="">
                            </div>
                            <div class="controls">
                                <input type="hidden" id="nama_ulos" name="nama_ulos" value="">
                            </div>
                            <div class="controls">
                                <select id="harga_model" name="harga_model" class="form-control" disabled>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#model').change(function() {
                var id = $(this).val();
                $.ajax({
                    url: "<?php echo site_url('model/get_model'); ?>",
                    method: "POST",
                    data: {
                        id: id
                    },
                    ...
                    success: function(data) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            model = data[i].nama_model;
                            html += '<option value=' + data[i].harga_model + '>' + data[i].harga_model + ' Rupiah </option>';
                        }
                        $('#harga_model').html(html);

                        html = '';
                        var namaproduk = $("#nama_produk").val() + ' Corak ' + model;


                        $('#nama_ulos').val(namaproduk);
                    }
                });
                return false;
            });
            ...
            $('#model').change(function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        var model = $("#harga_model").val();
                        var produk_harga = $("#harga_per_produk").val();
                        var total = parseInt(model) + parseInt(produk_harga);

                        var html = '';
                        html += '<h2 value=' + '>' + 'Rp.' + total + '</h2>';

                        $('#harga_produk').html(html);
                    }
                });
                return false;
            });
            ...
            $('#model').change(function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        var html = '';
                        html += '<input value=' + total + '>';

                        $('#total_ulos').val(total);
                    }
                });
                return false;
            });
